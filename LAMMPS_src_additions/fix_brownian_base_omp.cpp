...
FixBrownianBaseOMP::FixBrownianBaseOMP(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg), ThrOMP(lmp, THR_FIX)
{
  time_integrate = 1;
  ...
  if (nthreads != comm->nthreads) {
    ...
    // random_thr[0] = random;
    random_thr[0] = new RanMars(Fix::lmp, seed + comm->me);
  }
}

/* ---------------------------------------------------------------------- */

int FixBrownianBaseOMP::setmask()
{
  int mask = 0;
...
FixBrownianBaseOMP::~FixBrownianBaseOMP()
{
...
void FixBrownianBaseOMP::reset_dt()
{
